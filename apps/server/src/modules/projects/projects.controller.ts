...
export class ProjectsController {
  ...
  getById = asyncHandler(async (req, res) => {
    const project = await this.projectsService.getById(req.params.id!, req.user!.businessId);
  ...
  update = asyncHandler(async (req, res) => {
    ...
    const project = await this.projectsService.update(req.params.id!, body, req.user!.id, req.user!.businessId);
    sendSuccess(res, project, "Project updated");
  });

  addMilestone = asyncHandler(async (req, res) => {
    const body = req.body as CreateMilestoneBody;
    const project = await this.projectsService.addMilestone(
      req.params.id!,
      body,
      req.user!.id,
      req.user!.businessId,
  ...
  addMaterialUsage = asyncHandler(async (req, res) => {
    ...
    const usages = await this.projectsService.addMaterialUsage(
      req.params.id!,
      body,
      req.user!.id,
      req.user!.businessId,
  ...
  listMaterialUsages = asyncHandler(async (req, res) => {
    const usages = await this.projectsService.listMaterialUsages(req.params.id!, req.user!.businessId);
  ...
  addLaborEntry = asyncHandler(async (req, res) => {
    ...
    const entries = await this.projectsService.addLaborEntry(
      req.params.id!,
      body,
      req.user!.id,
      req.user!.businessId,
  ...
  listLaborEntries = asyncHandler(async (req, res) => {
    const entries = await this.projectsService.listLaborEntries(req.params.id!, req.user!.businessId);
    sendSuccess(res, entries, "Labor entries retrieved");
  });

  addBill = asyncHandler(async (req, res) => {
    const body = req.body as CreateBillBody;
    const bills = await this.projectsService.addBill(req.params.id!, body, req.user!.id, req.user!.businessId);
    sendSuccess(res, bills, "Bill created", 201);
  });

  listBills = asyncHandler(async (req, res) => {
    const bills = await this.projectsService.listBills(req.params.id!, req.user!.businessId);
  ...
  getCosting = asyncHandler(async (req, res) => {
    const costing = await this.projectsService.getCosting(req.params.id!, req.user!.businessId);
    sendSuccess(res, costing, "Project costing retrieved");
  });

  getProgress = asyncHandler(async (req, res) => {
    const progress = await this.projectsService.getProgress(req.params.id!, req.user!.businessId);
