...
export class UsersController {
  ...
  create = asyncHandler(async (req, res) => {
    ...
    const user = await this.usersService.createUser(body, req.user!.id, {
      ipAddress: req.ip,
  ...
  update = asyncHandler(async (req, res) => {
    ...
    const user = await this.usersService.updateUser(req.params.id!, body, req.user!.id, {
      ipAddress: req.ip,
      userAgent: req.headers["user-agent"],
    });
  ...
  assignRole = asyncHandler(async (req, res) => {
    ...
    const user = await this.usersService.assignRole(req.params.id!, body.roleId, req.user!.id, {
      ipAddress: req.ip,
      userAgent: req.headers["user-agent"],
    });
  ...
  deactivate = asyncHandler(async (req, res) => {
    const user = await this.usersService.deactivateUser(req.params.id!, req.user!.id, {
      ipAddress: req.ip,
      userAgent: req.headers["user-agent"],
    });
