...
export class FinanceController {
  ...
  listBankAccounts = asyncHandler(async (req, res) => {
    ...
    const accounts = await this.financeService.listBankAccounts(activeOnly);
    sendSuccess(res, accounts, "Bank accounts retrieved");
  });

  getBankAccount = asyncHandler(async (req, res) => {
    const account = await this.financeService.getBankAccount(req.params.id!);
  ...
  updateBankAccount = asyncHandler(async (req, res) => {
    ...
    const account = await this.financeService.updateBankAccount(req.params.id!, body, req.user!.id);
    sendSuccess(res, account, "Bank account updated");
  });

  deleteBankAccount = asyncHandler(async (req, res) => {
    await this.financeService.deleteBankAccount(req.params.id!, req.user!.id);
  ...
  updateInvoice = asyncHandler(async (req, res) => {
    ...
    const invoice = await this.financeService.updateInvoice(req.params.id!, body, req.user!.id);
  ...
  recordInvoicePayment = asyncHandler(async (req, res) => {
    ...
    const invoice = await this.financeService.recordInvoicePayment(req.params.id!, body, req.user!.id);
  ...
  getExpense = asyncHandler(async (req, res) => {
    const expense = await this.financeService.getExpense(req.params.id!);
  ...
  updateExpense = asyncHandler(async (req, res) => {
    ...
    const expense = await this.financeService.updateExpense(req.params.id!, body, req.user!.id);
    sendSuccess(res, expense, "Expense updated");
  });

  recordExpensePayment = asyncHandler(async (req, res) => {
    const body = req.body as CreatePaymentBody;
    const expense = await this.financeService.recordExpensePayment(req.params.id!, body, req.user!.id);
  ...
  listPurchaseOrderPayments = asyncHandler(async (req, res) => {
    const payments = await this.financeService.listPurchaseOrderPayments(req.params.id!);
  ...
  getCashBook = asyncHandler(async (_req, res) => {
    const entries = await this.financeService.getCashBook();
    sendSuccess(res, entries, "Cash book retrieved");
  });

  getBankBook = asyncHandler(async (req, res) => {
    const entries = await this.financeService.getBankBook(req.params.bankAccountId!);
