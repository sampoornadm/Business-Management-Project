...
export function usePurchaseOrders(query: ListPurchaseOrdersQuery) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // listPurchaseOrdersQuerySchema expects a single JSON-encoded string instead (it
      // JSON.parses `filters` before validating), so it has to be pre-stringified here rather
      // than left to axios's default serialization. See use-tenders.ts for the same gotcha.
